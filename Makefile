# ============================================================
#                 AlthenosOS - Makefile
# ============================================================

CONFIG_FILE = Makefile.config

include $(CONFIG_FILE)

# ------------------------------------------------------------
#   Target principal
# ------------------------------------------------------------
all: dirs bzImage iso

# ============================================================
#   Generar ISO estilo Linux
# ============================================================
iso: bzImage
	$(Q)mkdir -p $(ISO_DIR)/boot/grub
	$(Q)cp bzImage $(ISO_DIR)/boot/bzImage
	$(Q)echo 'set timeout=0'                      > $(ISO_DIR)/boot/grub/grub.cfg
	$(Q)echo 'set default=0'                     >> $(ISO_DIR)/boot/grub/grub.cfg
	$(Q)echo ''                                  >> $(ISO_DIR)/boot/grub/grub.cfg
	$(Q)echo 'menuentry "Althenos-OS-$(VERSION)" {' >> $(ISO_DIR)/boot/grub/grub.cfg
	$(Q)echo '  multiboot /boot/bzImage'         >> $(ISO_DIR)/boot/grub/grub.cfg
	$(Q)echo '  boot'                             >> $(ISO_DIR)/boot/grub/grub.cfg
	$(Q)echo '}'                                 >> $(ISO_DIR)/boot/grub/grub.cfg
	@printf "$(GREEN)  MKISO   $(RESET)AlthenosOS-$(VERSION).iso\n"
	$(Q)grub-mkrescue --output=AlthenosOS-$(VERSION).iso $(ISO_DIR) >/dev/null 2>&1
	$(Q)rm -rf $(ISO_DIR)

# ============================================================
#   Reglas de compilación (CC / AS / LD con pretty-print)
# ============================================================
$(BUILD_DIR)/drivers/%.o: $(SRC_DIR)/drivers/%.c
	@$(cmd_cc)
	$(Q)$(CC) $(CFLAGS) -I$(INCLUDE_DIR) -c $< -o $@

$(BUILD_DIR)/shell/%.o: $(SRC_DIR)/shell/%.c
	@$(cmd_cc)
	$(Q)$(CC) $(CFLAGS) -I$(INCLUDE_DIR) -c $< -o $@

$(BUILD_DIR)/kernel/%.o: $(SRC_DIR)/kernel/i386/%.c
	@$(cmd_cc)
	$(Q)$(CC) $(CFLAGS) -I$(INCLUDE_DIR) -c $< -o $@

$(BUILD_DIR)/bin/%.o: $(SRC_DIR)/bin/%.c
	@$(cmd_cc)
	$(Q)$(CC) $(CFLAGS) -I$(INCLUDE_DIR) -c $< -o $@

$(BUILD_DIR)/boot/%.o: $(SRC_DIR)/boot/%.s
	@$(cmd_as)
	$(Q)$(AS) $(ASFLAGS) -o $@ $<

$(BUILD_DIR)/initrd.tar: 
	$(Q)tar -cvf $(BUILD_DIR)/initrd.tar -C src/initrd . >/dev/null 2>&1

# Convertir initrd.tar en .o
$(OBJ_INITRD): $(BUILD_DIR)/initrd.tar
	@$(cmd_ld)
	$(Q)ld -m elf_i386 -r -b binary $(SRC_INITRD) -o $(OBJ_INITRD)

# ============================================================
#   Link final (bzImage)
# ============================================================
bzImage: $(OBJ_DRIVERS) $(OBJ_SHELL) $(OBJ_KERNEL) $(OBJ_BOOT) $(OBJ_BIN) $(OBJ_INITRD) $(SRC_DIR)/linker.ld
	@$(cmd_ld)
	$(Q)ld $(LDFLAGS) -T $(SRC_DIR)/linker.ld -o bzImage \
		$(OBJ_DRIVERS) $(OBJ_SHELL) $(OBJ_KERNEL) $(OBJ_BOOT) $(OBJ_BIN) $(OBJ_INITRD) >/dev/null 2>&1

# ============================================================
#   Directorios
# ============================================================
dirs:
	$(Q)mkdir -p $(BUILD_DIR)/kernel
	$(Q)mkdir -p $(BUILD_DIR)/boot
	$(Q)mkdir -p $(BUILD_DIR)/bin
	$(Q)mkdir -p $(BUILD_DIR)/shell
	$(Q)mkdir -p $(BUILD_DIR)/drivers

# ============================================================
#   Limpieza
# ============================================================
clean:
	@printf "$(YELLOW)  CLEAN   $(RESET)build/ iso/ bzImage\n"
	$(Q)rm -rf AlthenosOS-$(VERSION).iso bzImage initrd.tar $(BUILD_DIR) $(ISO_DIR)

# ============================================================
#   Ejecutar en QEMU
# ============================================================
run: all
	@printf "$(YELLOW)  RUN     $(RESET)QEMU\n"
	$(Q)qemu-system-i386 -cdrom AlthenosOS-$(VERSION).iso --enable-kvm
