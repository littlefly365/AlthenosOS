...
iso: bzImage
	...
	$(Q)cp src/kernel/boot/grub.cfg $(ISO_DIR)/boot/grub/grub.cfg
...
$(BUILD_DIR)/drivers/%.o: $(KERNEL_DIR)/drivers/%.c
	@$(cmd_cc)
	$(Q)$(CC) $(CFLAGS) -I$(INCLUDE_DIR) -c $< -o $@ >/dev/null 2>&1
...
$(BUILD_DIR)/bin/%.o: $(KERNEL_DIR)/bin/%.cpp
	@$(cmd_cpp)
	$(Q)$(CPP) $(CFLAGS) -I$(INCLUDE_DIR) -c $< -o $@ >/dev/null 2>&1

$(BUILD_DIR)/boot/%.o: $(KERNEL_DIR)/boot/%.S
	@$(cmd_as)
	$(Q)$(AS) $(ASFLAGS) -o $@ $< >/dev/null 2>&1
...
$(OBJ_INITRD): $(BUILD_DIR)/initrd.tar
	@$(cmd_ld)
	$(Q) $(LD) $(LDFLAGS) -r -b binary $(SRC_INITRD) -o $(OBJ_INITRD) >/dev/null 2>&1
...
clean:
	...
	$(Q)rm -rf AlthenosOS-$(VERSION).iso bzImage initrd.tar $(BUILD_DIR) $(ISO_DIR) >/dev/null 2>&1
